...
export default function DietPage() {
  ...
  // ❌ 제거: const [showPointToast, setShowPointToast] = useState(false);
  // ❌ 제거: const [showSavedToast, setShowSavedToast] = useState(false);

  // ❌ 제거: location.state 감지 useEffect (전체 32줄)
  ...
  return (
    <div className="bg-neutral-50 h-screen w-full flex flex-col overflow-hidden">
      ...
      {/* ❌ 제거: <PointToast show={showPointToast} points={50} /> */}
      {/* ❌ 제거: <PointToast show={showSavedToast} message="저장되었습니다" /> */}
    </div>
  );
}
...
//   const displayDate = isToday
//     ? "오늘의 식단"
//     : `${dateObj.getMonth() + 1}월 ${dateObj.getDate()}일 식단`;
...
//   useEffect(() => {
//     console.log(
//       "DietPage useEffect - location.state:",
//       location.state,
//     );

//     // Check if we should show toast
//     if (location.state?.showToast) {
//       console.log("Showing 50P toast");
//       setShowPointToast(true);
//       setTimeout(() => {
//         setShowPointToast(false);
//       }, 3000);
//     } else if (location.state?.newRecord) {
//       console.log("Showing saved toast");
...
//     if (
//       location.state?.newRecord ||
//       location.state?.showToast
//     ) {
//       setSelectedDate(getTodayDate());
//       setRecords(getDietRecordsByDate(getTodayDate()));
//       // Clear the state
//       navigate(location.pathname, { replace: true, state: {} });
//     }
//   }, [location, navigate]);

//   // Update records when selected date changes
//   useEffect(() => {
//     setRecords(getDietRecordsByDate(selectedDate));
//   }, [selectedDate]);
